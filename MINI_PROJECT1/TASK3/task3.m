...
% Plot Data Average Packet Delay
subplot(1,2,1);  % 1 row, 2 columns, first plot
bar(n_voip, media_APD_data, 'FaceColor', data_color);  % Data in blue
hold on;
errorbar(n_voip, media_APD_data, media_APD_data - errLow_APD_data, errHigh_APD_data - media_APD_data, 'k', 'linestyle', 'none', 'linewidth', 1.5);
title('Average Packet Delay - Data');
xlabel('Number of VoIP flows');
ylabel('Packet delay (ms)');
grid on;

% Plot VoIP Average Packet Delay
subplot(1,2,2);  % 1 row, 2 columns, second plot
bar(n_voip, media_APD_VoIP, 'FaceColor', voip_color);  % VoIP in orange
hold on;
errorbar(n_voip, media_APD_VoIP, media_APD_VoIP - errLow_APD_VoIP, errHigh_APD_VoIP - media_APD_VoIP, 'k', 'linestyle', 'none', 'linewidth', 1.5);
...
% Adjust layout for side-by-side display
sgtitle('Comparison of Average Packet Delay (Data vs VoIP)');  % Title for both subplots
